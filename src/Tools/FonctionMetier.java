...
public class FonctionMetier  implements IMetier {
    ...
    public ArrayList<Region> GetAllRegions() {
             ...
             ArrayList<Region>mesRegions = new ArrayList<>();
        try {
            
            Connection cnx = ConnexionBDD.getCnx();
            PreparedStatement ps = cnx.prepareStatement("select * from region");
            System.out.println(ps);
            ResultSet rs = ps.executeQuery();
            while(rs.next()){
                Region r = new Region(rs.getString(1),rs.getString(2),rs.getString(3));
                mesRegions.add(r);
